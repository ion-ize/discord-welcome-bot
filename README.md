...
- RANDOMIZE_GOODBYE_MESSAGES
  - Set to `true` to enable randomized goodbye messages from the list below. Defaults to `false`.
- GOODBYE_MESSAGES_LIST
  - A list of goodbye messages to be chosen from randomly if `RANDOMIZE_GOODBYE_MESSAGES` is enabled. Messages should be separated by `;;`. This has priority over the single `GOODBYE_MESSAGE` if enabled. It accepts the same variables. Example:
```**{member_name}** has left.;;See you later, {member_name}!```
...
- RANDOMIZE_QUICK_LEAVE_GOODBYE_MESSAGES
  - Set to `true` to enable randomized quick leave goodbye messages from the list below. Defaults to `false`.
- QUICK_LEAVE_GOODBYE_MESSAGES_LIST
  - A list of quick leave goodbye messages to be chosen from randomly if `RANDOMIZE_QUICK_LEAVE_GOODBYE_MESSAGES` is enabled. Messages should be separated by `;;`. This has priority over the single `QUICK_LEAVE_GOODBYE_MESSAGE` if enabled. It accepts the same variables. Example:
```**{member_name}** left quickly.;;Bye {member_name}!```
